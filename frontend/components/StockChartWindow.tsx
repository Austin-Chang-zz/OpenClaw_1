...
interface Props {
  ...
  onOpenDaily?: (symbol: string, stockName: string | null) => void;
...
export default function StockChartWindow({
  ...
  const [defaultSize] = useState(() => {
    if (typeof window === "undefined") return { x: 0, y: 68, width: 940, height: 520 };
    const hh = 68;
    return { x: 0, y: hh, width: Math.round(window.innerWidth * 0.4), height: window.innerHeight - hh };
  });
            ...
            {showMenu && (
              <div className="absolute right-0 top-7 bg-white dark:bg-slate-700 rounded-lg shadow-xl border border-slate-200 dark:border-slate-600 py-1 min-w-[172px] z-10">
                ...
                <button
                  className="w-full text-left px-3 py-2 text-xs text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors"
                  onMouseDown={(e) => e.stopPropagation()}
                  onClick={() => { onOpenDaily?.(symbol, data?.stock_name ?? stockName); setShowMenu(false); }}
                >📊 Open Daily Chart</button>
              </div>
            )}
          </div>
          <button
            onMouseDown={(e) => e.stopPropagation()}
            onClick={onMinimize}
            className="w-6 h-6 flex items-center justify-center rounded hover:bg-slate-600 text-slate-400 hover:text-white transition-colors text-lg leading-none"
            title="Minimize"
          >−</button>
          <button
            onMouseDown={(e) => e.stopPropagation()}
            onClick={onClose}
            className="w-6 h-6 flex items-center justify-center rounded hover:bg-slate-600 text-slate-300 hover:text-white transition-colors text-base leading-none"
            title="Close"
          >✕</button>
        </div>
      </div>
  ...
  // ── Minimized: small pill at top-left corner ──────────────────────────────
  if (isMinimized) {
    const topPx = 72 + minimizedIdx * 34;
    return (
      <div
        className="fixed left-2 pointer-events-auto"
        style={{ top: topPx, zIndex }}
        onMouseDown={() => { onFocus?.(); onBringChartsToFront?.(); }}
      >
        <div className="flex items-center gap-1.5 bg-slate-800 border border-slate-600 text-white rounded-lg px-2.5 py-1.5 shadow-lg select-none cursor-pointer">
          <span className="text-[11px] font-mono font-bold">W·{symbol}</span>
          <button
            onMouseDown={(e) => e.stopPropagation()}
            onClick={onMinimize}
            className="w-4 h-4 flex items-center justify-center rounded hover:bg-slate-600 text-slate-300 text-[10px]"
            title="Restore"
          >□</button>
          <button
            onMouseDown={(e) => e.stopPropagation()}
            onClick={onClose}
            className="w-4 h-4 flex items-center justify-center rounded hover:bg-red-700 text-slate-300 text-[10px]"
            title="Close"
          >✕</button>
        </div>
      </div>
